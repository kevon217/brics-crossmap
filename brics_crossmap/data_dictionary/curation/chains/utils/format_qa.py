...
def cross_encoder_rerank(df, model_name, variable_name, top_k):
    ...
    title_combinations = [
        [variable_title, title] for title in df["data element concept names"]
    ]
    definition_combinations = [
        [variable_definition, definition]
        for definition in df["data element concept definitions"]
    ]
...
def randomize_multiple_choice(df_filtered):
...
def create_qa_prompt_from_df(df_filtered):
    ...
    options = []
    df_filtered = randomize_multiple_choice(df_filtered)
