...
def check_set_device(cfg):
    ...
    crossmap_logger.info(f"Running on {device}")
...
def tokenize_columns(df, columns, model_name):
    ...
    for col in columns:
        crossmap_logger.info(f"Tokenizing {col}")
...
def hybrid_builder(
    ...
    for col in embed_columns:
        crossmap_logger.info(f"Embedding {col}")
        batch = df[col].values.tolist()
        ...
        for i in range(0, len(batch), sparse_batch_size):
            crossmap_logger.info(f"Embedding {i} to {i + sparse_batch_size}")
